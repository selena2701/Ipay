...
public class ReportsRepo {
    ...
    private void loadReportsFromDB() throws SQLException, ClassNotFoundException {
        ...
        String query="SELECT CAST(MONTH(DatePaid)AS varchar) +'-'+ CAST(YEAR(DatePaid)AS varchar) AS \"PERIOD\", COUNT(ElecBillId) AS \"numberTransactions\", SUM(Total) AS \"totalTransactionValue\"FROM E_ELECTRICITY_BILL EB, E_CUSTOMER CUS WHERE EB.CusId=CUS.CusId AND EB.StatusBill='PAID' GROUP BY CAST(MONTH(DatePaid)AS varchar) +'-'+ CAST(YEAR(DatePaid)AS varchar)";
